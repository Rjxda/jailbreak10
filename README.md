...
## (2) Adding support for other devices
 ...
 1. [Download the IPSW for your device](http://ipsw.me). The bugs are there in any version <= 10.1.1 but the further back you go the more offsets will be wrong so ideally stick to 10.1.1 (and for anything earlier that iOS 10, the kernel cache
...
```
 ...
 5. Run a command like: `lzssdec -o 0x1b4 < kernel.release.n51 > kernel.decompressed`

 6. Open the decompressed kernelcache in a recent version of IDA Pro (with support for iOS kextcaches)

 7. Say yes when IDA asks to split by kext

 8. Let the auto-analysis run -- depending on how fast your computer is this might take a while!

 9. Click View -> Open Subviews -> Segments and find the `__TEXT:HEADER` segment, the start should be `FFFFFFF007004000`. If it isn't, note this down as you'll need to work out a couple of offsets relative to this.

 10. Click View -> Open Subviews -> Names and find the kernproc data symbol.

 11. Subtract the `__TEXT:HEADER` value from that. This is the kernproc offset (e.g. for iPhone 5S 10.1.1, kernproc is at `FFFFFFF0075AE0E0` making the offset: `0x5AA0E0`)

 12. Now the harder one! We need to find allproc which isn't exported so is harder to find:

    * Click View -> Open Subviews -> Strings and find the string "pgrp_add : pgrp is dead adding process"
    ...
    * Open `offset.c` and add support for your device. You should only have to update those two variable (kernproc and allproc). The structure offsets should stay the same, at least for recent kernels. If you want to target a much older kernel you'll also have to work out all the structure offsets -- this is much more fiddly.

### 32-bit devices:
All the offsets will be totally different and the code which manipulates the kernel data structures will also be completely wrong. There's no reason it wouldn't work but you'll have to fix the code to make it work

### Fixing userspace stuff
I also rely on a handful of offsets in amfid; you should be able to find those very easily if they're different on your target. See the code and alse the section "Patch amfid" below.
